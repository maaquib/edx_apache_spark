# Databricks notebook source exported at Mon, 6 Jul 2015 20:31:37 UTC
# MAGIC %md
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return (labelsAndPreds.map(lambda a: squaredError(a[0], a[1])).reduce(lambda a, b: a+b) / float(labelsAndPreds.count()))**0.5
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda a: gradientSummand(w, a))
        
        # Update the weights
        alpha_i = alpha / (n * np.sqrt(i+1))
        w -= alpha_i * gradient.reduce(lambda a, b: a+b)
    return w, errorTrain
...
labelsAndPreds = parsedValData.map(lambda a: getLabeledPrediction(np.zeros(d), a))
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, numIters, alpha, miniBatchFrac, np.zeros(d), reg, regType, useIntercept)
...
def twoWayInteractions(lp):
    ...
    """
    return LabeledPoint(lp.label, np.hstack((lp.features, np.array([x*y for x in lp.features for y in lp.features]))))
