...
wordsGrouped = wordPairs.map(lambda word: word).groupByKey()
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda a: (a[0], sum(list(a[1]))))
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordsRDD.map(lambda word: (word, 1)).reduceByKey(lambda a, b: a+b).map(lambda a: 1).reduce(lambda a, b:a+b)
print uniqueWords
...
# TODO: Replace <FILL IN> with appropriate code
shakeWordsRDD = shakespeareWordsRDD.flatMap(lambda a: a.split())
