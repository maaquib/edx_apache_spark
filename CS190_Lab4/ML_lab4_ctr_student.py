# Databricks notebook source exported at Tue, 14 Jul 2015 15:39:02 UTC
# MAGIC %md
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    return SparseVector(numOHEFeats, [(OHEDict.get(a), 1.) for a in rawFeats])
...
# Cache the data
rawTrainData.cache();rawValidationData.cache();rawTestData.cache()
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    return SparseVector(numOHEFeats, [(OHEDict.get(a), 1.) for a in rawFeats if a in OHEDict])
...
classOneFracTrain = OHETrainData.filter(lambda a: a.label==1.0).count() / float(OHETrainData.count())
print classOneFracTrain

logLossTrBase = OHETrainData.map(lambda a: computeLogLoss(classOneFracTrain, a.label)).reduce(lambda a, b: a+b) / float(OHETrainData.count())
...
def getP(x, w, intercept):
    ...
    """
    rawPrediction = 1 / float(1 + exp(-(w.dot(x) + intercept)))
...
def evaluateResults(model, data):
    ...
    """
    return data.map(lambda a: computeLogLoss(getP(a.features, model.weights, model.intercept), a.label)).reduce(lambda a, b: a+b) / float(data.count())
...
# TODO: Replace <FILL IN> with appropriate code
logLossValBase = OHEValidationData.map(lambda a: computeLogLoss(classOneFracTrain, a.label)).reduce(lambda a, b: a+b) / float(OHEValidationData.count())
...
def computeSparsity(data, d, n):
    ...
    """
    return (data.map(lambda a: len(a.features.values)).reduce(lambda a, b: a+b) / float(d * n))
...
logLossTestBaseline = hashTestData.map(lambda a: computeLogLoss(classOneFracTrain, a.label)).reduce(lambda a, b: a+b) / float(hashTestData.count())
