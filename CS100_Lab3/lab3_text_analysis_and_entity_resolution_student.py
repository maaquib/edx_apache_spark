...
def tokenize(string):
    ...
    """
    return [val for val in simpleTokenize(string) if val not in stopwords]
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda a: (a[0], a[1])).reduceByKey(lambda a, b: a if (len(a) > len(b)) else b).takeOrdered(1, lambda s: -1 * len(s[1]))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.map(lambda a: list(set(a[1])))
    tokenCountPairTuple = uniqueTokens.flatMap(lambda a: a).map(lambda a: (a, 1))
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a+b)
    return (tokenSumPairTuple.map(lambda a: (a[0], float(N)/a[1])))
...
def dotprod(a, b):
    ...
    """
    return sum(a.get(k, 0) * b.get(k, 0) for k in list(set(a.keys() + b.keys())))
...
def cosineSimilarity(string1, string2, idfsDictionary):
    ...
    """
    w1 = tfidf(tokenize(string1.lower()), idfsDictionary)
    w2 = tfidf(tokenize(string2.lower()), idfsDictionary)
    return cossim(w1, w2)
...
similaritiesBroadcast = (crossSmall
                         .map(lambda a: computeSimilarityBroadcast(a))
                         .cache())
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda a: ("%s %s" % (a[1], a[0]), a[2]))

trueDupsRDD = (sims
               .join(goldStandard).map(lambda a: (a[0], a[1][0])))
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.map(lambda a: a[1]).reduce(lambda a, b: a+b) / float(trueDupsCount)

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = nonDupsRDD.map(lambda a: a[1]).reduce(lambda a, b: a+b) / float(sims.count() - trueDupsCount)
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda a: (a[0], tfidf(a[1], idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda a: (a[0], tfidf(a[1], idfsFullBroadcast.value)))
...
amazonInvPairsRDD = (amazonWeightsRDD
                    .map(lambda a: invert(a)).flatMap(lambda a: a)
                    .cache())
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(lambda a: swap(a)).groupByKey()
                .cache())
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = sum(amazonWeightsBroadcast.value.get(i, 0) * googleWeightsBroadcast.value.get(i, 0) for i in tokens)
    value = s / (norm(amazonNormsBroadcast.value[amazonRec]) * norm(googleNormsBroadcast.value[googleRec]))
    key = (amazonRec, googleRec)
    return (key, value)

similaritiesFullRDD = (commonTokens
                       .fastCosineSimilarity(lambda a: fastCosineSimilarity(a))
                       .cache())
