...
not200 = access_logs.filter(lambda a: a.response_code != 200)
...
hosts = access_logs.map(lambda a: (a.host, 1)).reduceByKey(lambda a, b : a + b)

uniqueHosts = hosts.map(lambda a: a[0])
...
dayToHostPairTuple = access_logs.map(lambda a: ((a.date_time.day, a.host), 1)).reduceByKey(lambda a, b: a+b)

dayGroupedHosts = dayToHostPairTuple.map(lambda a: a[0]).map(lambda a: (a[0], 1))
...
dayAndHostTuple = access_logs.map(lambda a: ((a.date_time.day, a.host), 1))

groupedByDay = dayAndHostTuple.map(lambda a: a[0]).map(lambda a: (a[0], 1))

sortedByDay = groupedByDay.reduceByKey(lambda a, b: a+b).sortByKey()

avgDailyReqPerHost = (sortedByDay.join(dailyHosts).map(lambda a: (a[0], a[1][0]/a[1][1])).sortByKey()
                      .cache())
...
badRecords = (access_logs.filter(lambda a: a.response_code == 404).cache())
...
badEndpoints = badRecords.map(lambda a: (a.endpoint, 1))
...
badEndpointsCountPairTuple = badRecords.map(lambda a: (a.endpoint, 1))
...
errHostsCountPairTuple = badRecords.map(lambda a: (a.host, 1))

errHostsSum = errHostsCountPairTuple.reduceByKey(lambda a, b: a+b)

errHostsTop25 = errHostsSum.sortByKey().takeOrdered(25, lambda s: -1 * s[1])
...
errDateCountPairTuple = badRecords.map(lambda a: (a.date_time.day, 1))
